...
class DBOperator(object):
    ...
    def __init__(self):
        ...
        global INIT_SUCCESS
        try:
            self.db = sqlite3.connect('./ChessGame.db')
...
class MyGrid(tkinter.Canvas):
    ...
    def __init__(self, master=None, width=50, height=50, col_num=20, row_num=6, match_degree=None):
        ...
        for x in range(col_num):
            for y in range(row_num):
                ...
                self.create_rectangle((x+0.5)*width, (y+0.5)*height, (x + 1.5)*width, (y + 1.5)*width)
        if match_degree:
            global ITEM_SIZE
            # 展示匹配度
    ...
    def key_clicked(self, event):
        ...
        if str(event.char).strip() == "":
            ...
            if self.is_checked:
                # 删除红框 如果存在圆圈则移除圆圈
                for x_num, y_num in self.red_circle:
                ...
                for x_num, y_num in self.blue_circle:
                    if self.checked_pos == self.get_pos_by_xy_num(x_num=x_num, y_num=y_num):
...
class SmallWindow(tkinter.Frame):
    ...
    def __init__(self, master=None, button_text="确认", width=50, height=50, col_num=20, row_num=6, match_degree=None, game=None, button_two=None, button_three=None):
        ...
        self.circle.pack()
        # 创建按钮
        if button_three or button_three:
            self.button = tkinter.Button(master=self, text=button_text, command=self.button_clicked, height=1)
        ...
        if button_two:
            print("two")
            self.button2 = tkinter.Button(master=self, text=button_two, command=self.button_clicked_clear, height=1)
            self.button2.pack(pady=3)
        if button_three:
            print("three")
            self.button3 = tkinter.Button(master=self, text=button_three, command=self.button_clicked_save, height=1)
